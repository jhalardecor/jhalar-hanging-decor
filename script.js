...
function renderFilters(){const el=$('#product-filters');el.innerHTML=['all',...categories()].map(c=>'<button class="filter-btn '+(c===state.filter?'active':'')+'" data-filter="'+esc(c)+'" aria-pressed="'+(c===state.filter?'true':'false')+'">'+esc(c==='all'?'All designs':c)+'</button>').join('');el.onclick=e=>{const b=e.target.closest('[data-filter]');if(!b)return;const next=b.dataset.filter;if(next===state.filter)return;state.filter=next;state.expanded=next!=='all';el.querySelectorAll('[data-filter]').forEach(btn=>{const selected=btn===b;btn.classList.toggle('active',selected);btn.setAttribute('aria-pressed',String(selected));});renderProducts();requestAnimationFrame(()=>{const isMobile=window.matchMedia('(max-width: 760px)').matches;if(isMobile){const next=b.nextElementSibling;if(next){const rail=el;const targetLeft=next.offsetLeft-(rail.clientWidth-next.offsetWidth)/2;rail.scrollTo({left:Math.max(0,targetLeft),behavior:'smooth'});}else{const grid=$('#product-grid');if(grid)grid.scrollIntoView({behavior:'smooth',block:'start'});}}else{const target=$('#product-grid');if(target){const y=target.getBoundingClientRect().top+window.scrollY-92;window.scrollTo({top:Math.max(0,y),behavior:'smooth'});}}const target=$('#product-grid');if(target){target.classList.remove('catalogue-pulse');void target.offsetWidth;target.classList.add('catalogue-pulse');}});}}
